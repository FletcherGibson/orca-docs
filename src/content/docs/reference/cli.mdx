...
- `agent-orca work --worker <id> [--mode manual|auto|external] [--role coder] [--timeout-seconds 1800] [--cleanup-worktree]` — lease next queued task and execute it
  - `manual` mode waits for your commit, then verifies
  - `auto` / `external` execute the configured role from `orca.yaml` (default role name: `coder`) and expect a commit to be created
...
- `agent-orca doctor` — environment sanity checks (git repo, worktree, go, and role executables from `orca.yaml`)
